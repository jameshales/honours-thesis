...
We need to show that $M_s \entails \somerefs_a \bigwedge_{b \in B}
\covers_b \Gamma_b$. To do this we follow the same strategy as for proving {\bf
RK}: we construct an $a$-refinement $N_t$, and show that $N_t \entails
...
Let $t$ be a state not in $S^M$ or any of the $N^{b,\gamma}$. Then we construct a
Kripke model $N = (S^N, R^N, V^N)$ where:
...
can use the equivalences from Lemma~\ref{k-equivs} to push the $\somerefs_a$
