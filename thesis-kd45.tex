\chapter{Doxastic logic}\label{kd45}

In this chapter we provide an axiomatisation for the refinement quantified
doxastic logic, over the class of \classKD{}. The axiomatisation for the
doxastic variant differs from the basic modal variant due to the interpretation
of the refinement operator. As we previously remarked in
Chapter~\ref{semantics}, the doxastic variant is not a conservative extension of
the basic modal variant, and so any axiomatisation cannot simply be an extension
of the axiomatisation for the basic modal variant.

\section{Technical preliminaries}

In previous work, we gave an axiomatisation of the single-agent variant of the
refinement quantified doxastic logic. Similar to the axiomatisation for the
single-agent, basic modal variant, the axiomatisation relied on a special form
for the well-formed formulae of doxastic logic: what we called a prenex normal
form. The prenex normal form prohibits modal operators ($\knows$ and
$\suspects$) from appearing within other modal operators. The prenex normal form
simplified the axiomatisation and proof of soundness, by avoiding complications
that would arise due to the transitivity of doxastic models, when nested
modalities are present.

For the axiomatisation of the multi-agent variant, we introduce a disjunctive
normal form that serves the same purpose for the multi-agent variant as the
prenex normal form did for the single-agent variant. It is not possible to
express every multi-agent doxastic formula as an equivalent formula without
nested modalities; for example, formulae such as $\knows_a \knows_b p$ cannot be
expressed without such nestings. However, it is possible to express multi-agent
doxastic formulae as an equivalent formula without direct nestings of modalities
that belong to the same agent. So for example, formulae such as $\knows_a
\knows_b p$ are allowed, but formulae such as $\knows_a \knows_a p$ are not.
This allows us to avoid the same complications due to transitivity that the
...
We note that, similar to the prenex normal form used previously, our initial
formulation of the disjunctive normal form is not in terms of the cover
operator. This is in contrast to the disjunctive normal form used for the basic
modal logic, however we will later introduce a cover logic disjunctive normal
form which is similar to both.
...
$$
\alpha ::= \pi \land \bigwedge_{b \in B \subseteq A - \{a\}} \covers_b \Gamma_b \bnfalt
\alpha \lor \alpha
$$
...
$$
\alpha ::= \pi \land \bigwedge_{a \in B \subseteq A} \covers_a \Gamma_a \bnfalt
\alpha \lor \alpha
$$
...
Every formula of \lang{} is equivalent to a formula in cover disjunctive normal
...
a formula in $a$-cover disjunctive normal form. Thus the result of the
conversion is a formula in cover disjunctive normal form.
\end{proof}

The cover disjunctive normal form will be used in our completeness proofs.
...
\begin{proof}
The soundness of the axioms {\bf P}, {\bf K}, {\bf D}, {\bf 4}, and {\bf 5} and
the rules {\bf MP} and {\bf NecK} can be shown by the same reasoning used to
show that they are sound in basic doxastic logic. The soundness of the axioms
{\bf RP} and {\bf R}, and the rule {\bf NecR} can be shown by the same reasoning
