...
\phi \implies \suspects_a p \land \knows_b \suspects_a \neg p &
\text{(Definition of $\suspects$)}\\
\phi \implies \suspects_a p \land \knows_b (\suspects_a \neg p \land \suspects_a
\top)&
\text{(Consequence of {\bf D})}\\
\phi \implies \suspects_a \neg \neg p \land \knows_b (\suspects_a \neg \neg \neg
p \land \suspects_a \neg \neg \top)&
\text{({\bf P})}\\
\phi \implies \suspects_a \neg \allrefs_a \neg p \land \knows_b (\suspects_a \neg
\allrefs_a \neg \neg p \land \suspects_a \neg \allrefs_a \neg \top)&
\text{({\bf RP})}\\
\phi \implies \suspects_a \somerefs_a p \land \knows_b (\suspects_a \somerefs_a
\neg p \land \suspects_a \somerefs_a \top)&
\text{(Definition of $\somerefs$)}\\
\phi \implies \somerefs_a \covers_a \{p\} \land \knows_b (\suspects_a \somerefs_a
\neg p \land \suspects_a \somerefs_a \top)&
\text{({\bf RKD45})}\\
\phi \implies \somerefs_a \covers_a \{p\} \land \knows_b \somerefs_a \covers_a
\{\neg p, \top\}&
\text{({\bf RKD45})}\\
\phi \implies \somerefs_a \covers_a \{p\} \land \covers_b \{ \somerefs_a \covers_a
\{\neg p, \top\} \}&
\text{(Definition of $\covers$)}\\
\phi \implies \somerefs_a \covers_a \{p\} \land \somerefs_a \covers_b \{
\covers_a \{\neg p, \top\} \}&
\text{({\bf RComm})}\\
\phi \implies \somerefs_a (\covers_a \{p\} \land \covers_b \{
\covers_a \{\neg p, \top\} \})&
\text{({\bf RDist})}\\
\phi \implies \somerefs_a (\knows_a p \land \knows_b 
\suspects_a \neg p)&
\text{(Definition of $\covers$)}\\
\phi \implies \somerefs_a (\knows_a p \land \knows_b 
\neg \knows_a p)&
...
\begin{figure}\label{kd45-diagram}
...
the soundness proofs for these axioms have been the main technical result.
...
property that we cannot combine models in the manner performed in the proof of
{\bf RKD45}, and expect them to continue to satisfy the formulae we wish them to
satisfy in the combined model.
