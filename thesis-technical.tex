...
multi-agent setting, where there is a set of agents, $A$, then modal logic
...
is said to represent belief and knowledge respectively. The interpetation in
terms of knowledge is quite intuitive; if an agent considers a set of possible
worlds, and some statement $\phi$ is true in all of the worlds that the agent
considers possible (that is, $\knows_a \phi$), then it can reasonably be said
that the agent {\em knows} that $\phi$.
...
statement, then they know or believe that this is the case.
